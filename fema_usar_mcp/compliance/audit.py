...
class ComplianceReport:
    ...
    def __init__(
        ...
        self.compliance_score: Optional[float] = None
    ...
    def calculate_compliance_score(self) -> float:
        ...
        score -= (total_findings - critical_findings - high_findings) * 2  # -2 points per other
...
class AuditLogger:
    ...
    async def _store_event_db(self, event: AuditEvent):
        ...
        try:
            ...
            session.execute(insert_sql, {
                ...
                "geolocation": json.dumps(event.geolocation) if event.geolocation else None,
                "device_info": json.dumps(event.device_info) if event.device_info else None,
    ...
    def _contains_sensitive_data(self, data: Dict[str, Any]) -> bool:
        ...
        sensitive_keys = {
            ...
            'pii', 'personal_info', 'medical_info', 'secret', 'key', 'token'
        }
    ...
    def _encrypt_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def encrypt_sensitive(obj):
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if key.lower() in {'password', 'ssn', 'social_security', 'secret', 'token'}:
...
class ComplianceReporter:
    ...
    async def generate_compliance_report(
        ...
        Returns:
            Compliance report
        """
        report_id = f"compliance_{framework.value}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
        ...
        await self.audit_logger.log_event(AuditEvent(
            ...
            event_data={"report_type": report.report_type, "period_days": (period_end - period_start).days},
            before_state=None,
            after_state=None,
            success=True,
            error_message=None,
    ...
    async def _analyze_fema_compliance(self, report: ComplianceReport, events: List[AuditEvent]):
        ...
        security_events = [e for e in events if e.event_type == AuditEventType.USER_LOGIN_FAILED]
    ...
    async def _analyze_fisma_compliance(self, report: ComplianceReport, events: List[AuditEvent]):
        """Analyze FISMA compliance."""
        # Check for configuration changes without approval
        config_changes = [e for e in events if e.event_type == AuditEventType.SYSTEM_CONFIG_CHANGE]
        ...
        sensitive_access = [e for e in data_access_events if e.data_classification in ["secret", "top_secret"]]
    ...
    async def _analyze_privacy_compliance(self, report: ComplianceReport, events: List[AuditEvent]):
        """Analyze Privacy Act compliance."""
        # Check for PII access
        pii_events = [e for e in events if e.event_type == AuditEventType.PRIVACY_DATA_ACCESS]
    ...
    def _generate_recommendations(self, findings: List[Dict[str, Any]]) -> List[str]:
        ...
        if "missing_audit_events" in finding_types:
            recommendations.append("Implement comprehensive audit logging for all system events")
            
        if "unauthorized_config_change" in finding_types:
            recommendations.append("Establish formal change management process with approval workflows")
            
        if "excessive_failed_logins" in finding_types:
            recommendations.append("Implement account lockout policies and monitoring for brute force attacks")
            
        if "unauthorized_pii_access" in finding_types:
            recommendations.append("Implement access controls and justification requirements for PII data")
...
class ComplianceManager:
    ...
    async def log_audit_event(
        ...
        **kwargs
        ...
        if not compliance_frameworks:
            compliance_frameworks = [ComplianceFramework.FEMA_DIRECTIVE, ComplianceFramework.NIMS_STANDARD]
    ...
    async def apply_retention_policy(self):
        ...
        cutoff_date = datetime.now(timezone.utc) - timedelta(days=max(self.retention_policies.values()))
        ...
        try:
            ...
            await self.log_audit_event(
                ...
                severity=AuditSeverity.INFO,
                event_data={"archived_count": archived_count, "cutoff_date": cutoff_date.isoformat()}
            )
