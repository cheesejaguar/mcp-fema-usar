...
class LoadBalancer:
    ...
    def __init__(self, strategy: LoadBalanceStrategy = LoadBalanceStrategy.HEALTH_WEIGHTED):
    ...
    def select_server(self, request_data: Dict[str, Any] = None) -> Optional[BackendServer]:
    ...
    def _hash_based_select(self, servers: List[BackendServer], request_data: Dict[str, Any]) -> BackendServer:
        """Hash-based server selection."""
        if not request_data:
            return servers[0]
            
        # Use user ID or IP for consistent hashing
        hash_key = request_data.get("user_id") or request_data.get("client_ip", "default")
    ...
    def _health_weighted_select(self, servers: List[BackendServer]) -> BackendServer:
        ...
        def calculate_score(server: BackendServer) -> float:
            ...
            connection_usage = (server.active_connections / server.max_connections) * 100
...
class APIGateway:
    ...
    async def _forward_request(self, request: web.Request, server: BackendServer) -> web.Response:
