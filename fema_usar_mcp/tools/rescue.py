...
def rescue_squad_operations(
    ...
    operation_type: Literal["extraction", "debris_removal", "stabilization", "reconnaissance"] = "extraction",
    ...
    try:
        ...
        standard_equipment = {
            "extraction": ["hydraulic_tools", "cribbing", "rope_systems", "medical_kit", "communications"],
            "debris_removal": ["chainsaws", "cutting_torch", "pneumatic_tools", "lifting_bags", "safety_gear"],
            "stabilization": ["shoring_materials", "hydraulic_jacks", "welding_equipment", "measuring_tools"],
            "reconnaissance": ["cameras", "measuring_devices", "safety_equipment", "communications", "lighting"]
        }
        ...
        operation_data = {
            ...
            "complexity_level": determine_operation_complexity(operation_type, personnel_assigned, victim_id),
            ...
            "estimated_completion": (datetime.now() + timedelta(hours=estimated_duration)).isoformat()
        }
        ...
        if not operation_data["personnel_adequate"]:
            recommendations.append(f"Increase personnel to minimum {min_personnel} for {operation_type} operations")
        
        if operation_type == "extraction" and victim_id:
            recommendations.append(f"Coordinate with Medical Team for victim {victim_id} treatment preparation")
            recommendations.append("Prepare extraction pathway and stabilize route before victim movement")
        
        if operation_data["safety_level"] == "HIGH_RISK":
            recommendations.append("Implement enhanced safety protocols and continuous monitoring")
        ...
        if "hydraulic_tools" in required_equipment:
            recommendations.append("Verify hydraulic system pressure and backup power availability")
        if "cutting_torch" in required_equipment:
            recommendations.append("Ensure proper ventilation and fire suppression measures")
        
        # Squad coordination recommendations
        recommendations.append(f"Coordinate with Search Team for victim location confirmation")
        recommendations.append(f"Maintain radio contact every 15 minutes during operation")
...
def victim_extraction_planner(
    ...
    extraction_method: Literal["manual", "mechanical", "technical", "complex"] = "manual",
    victim_condition: Literal["conscious", "unconscious", "critical", "deceased"] = "conscious",
    entrapment_type: Literal["debris", "structural", "confined_space", "vehicle"] = "debris",
    access_difficulty: Literal["clear", "limited", "complex", "extremely_difficult"] = "limited"
) -> str:
    ...
    try:
        ...
        extraction_phases = [
            {
                ...
                "duration_minutes": 30,
                "activities": ["Victim assessment", "Route planning", "Equipment setup"],
            ...
            },
            {
                ...
                "activities": ["Debris removal", "Structural stabilization", "Access pathway"],
            ...
            },
            {
                ...
                "duration_minutes": 20,
                "activities": ["Medical assessment", "Stabilization", "Extraction preparation"],
            ...
            },
            {
                ...
                "activities": ["Victim movement", "Medical monitoring", "Safety oversight"],
        ...
        equipment_lists = {
            "manual": [
                "Basic rescue tools", "Rope systems", "Cribbing materials", "Medical kit",
                "Communications", "Lighting", "Safety equipment"
            ],
            "mechanical": [
                "Hydraulic rescue tools", "Pneumatic tools", "Lifting systems", "Cutting equipment",
                "Rope systems", "Medical kit", "Monitoring equipment", "Safety gear"
            ],
            "technical": [
                "Advanced hydraulic systems", "Specialized cutting tools", "Rigging equipment",
                "Confined space gear", "Advanced medical kit", "Monitoring systems",
                "Technical rescue gear", "Communication systems"
            ],
            "complex": [
                "Heavy hydraulic systems", "Specialized extraction equipment", "Engineering tools",
                "Advanced rigging", "Complete medical setup", "Monitoring systems",
                "Technical rescue equipment", "Heavy lifting gear", "Specialized safety equipment"
            ]
        }
        
        required_equipment = equipment_lists.get(extraction_method, equipment_lists["manual"])
        
        # Calculate total operation time
        total_operation_time = sum(phase["duration_minutes"] for phase in extraction_phases)
        ...
        extraction_plan = {
            ...
            "resource_requirements": {
                ...
                "specialized_tools": len([eq for eq in required_equipment if "advanced" in eq.lower() or "specialized" in eq.lower()])
            },
            
            "extraction_phases": extraction_phases,
            
            "safety_considerations": generate_extraction_safety_considerations(entrapment_type, victim_condition, access_difficulty),
        ...
        if condition_info["urgency"] == "immediate":
            recommendations.append("IMMEDIATE ACTION REQUIRED - Critical victim condition")
            recommendations.append("Deploy advanced medical support simultaneously with extraction")
        
        if access_difficulty in ["complex", "extremely_difficult"]:
            recommendations.append("Consider alternative extraction routes")
            recommendations.append("Request structural specialist evaluation if not already present")
        
        if extraction_method in ["technical", "complex"]:
            recommendations.append("Deploy Technical Rescue Specialist with extraction team")
...
def structural_stabilization(
    ...
    stabilization_type: Literal["shoring", "cribbing", "lifting", "temporary_repair"] = "shoring",
    ...
    load_requirements: Optional[float] = None
) -> str:
    ...
    try:
        ...
        stabilization_specs = {
            ...
            "lifting": {
                "materials": ["Hydraulic jacks", "Lifting bags", "Steel plates", "Support posts"],
                "time_hours": 3,
                "personnel": 5,
                "complexity": "high",
            ...
            "temporary_repair": {
                "materials": ["Steel plates", "Welding equipment", "Bolts", "Structural adhesives"],
                "time_hours": 6,
                "personnel": 4,
                "complexity": "high",
        ...
        # Urgency multipliers
        urgency_factors = {
            "immediate": {"time_multiplier": 0.7, "personnel_multiplier": 1.5, "priority": 1},
            "planned": {"time_multiplier": 1.0, "personnel_multiplier": 1.0, "priority": 3},
            "preventive": {"time_multiplier": 1.2, "personnel_multiplier": 1.0, "priority": 4},
            "emergency": {"time_multiplier": 0.5, "personnel_multiplier": 2.0, "priority": 0}
        }
        ...
        required_personnel = int(specs["personnel"] * urgency_info["personnel_multiplier"])
        ...
        if load_requirements:
            ...
            if not adequate_capacity:
                recommended_method = determine_adequate_stabilization_method(load_requirements, stabilization_specs)
            else:
        ...
        stabilization_data = {
            ...
            "timeline": generate_stabilization_timeline(estimated_time, specs["materials"], required_personnel),
        ...
        if urgency == "emergency":
            recommendations.append("EMERGENCY STABILIZATION - Deploy immediately with all available personnel")
        ...
        if personnel_at_risk > 5:
            recommendations.append("Establish incident command post for stabilization operation")
            recommendations.append("Coordinate with Safety Officer for personnel protection")
...
def heavy_equipment_operations(
    equipment_type: Literal["excavator", "crane", "loader", "concrete_breaker", "compactor"] = "excavator",
    operation_mode: Literal["debris_removal", "lifting", "site_prep", "demolition", "material_handling"] = "debris_removal",
    location: str = "Site A, Grid Reference 15",
    operator_qualified: bool = True,
    equipment_condition: Literal["excellent", "good", "fair", "needs_maintenance"] = "good",
    ...
    try:
        ...
        equipment_specs = {
            ...
            "compactor": {
                "primary_uses": ["site_prep", "material_compaction", "surface_preparation"],
                "reach_feet": 8,
        ...
        operation_requirements = {
            ...
            "lifting": {
                "duration_hours": 4,
                "support_personnel": 4,
                "special_equipment": ["Rigging equipment", "Load calculations", "Spotters", "Safety gear"],
                "risk_level": "high"
            },
            "site_prep": {
                "duration_hours": 6,
            ...
            "demolition": {
                "duration_hours": 10,
                "support_personnel": 3,
                "special_equipment": ["Protective barriers", "Dust control", "Safety monitoring"],
                "risk_level": "high"
            },
            "material_handling": {
                "duration_hours": 6,
        ...
        operation_info = operation_requirements.get(operation_mode, operation_requirements["debris_removal"])
        
        # Calculate fuel requirements
        estimated_fuel_needed = specs["fuel_consumption_gph"] * operation_info["duration_hours"]
        ...
        operation_data = {
            ...
            "resource_requirements": {
                ...
                "equipment_inspection_current": equipment_condition in ["excellent", "good"],
                "safety_perimeter_established": work_area_secured
            },
            
            "safety_protocols": generate_heavy_equipment_safety_protocols(equipment_type, operation_mode, specs["safety_radius_feet"]),
        ...
        if operation_mode == "lifting":
            recommendations.append("Deploy Heavy Equipment/Rigging Specialist for load calculations")
            recommendations.append("Conduct lift plan review and approval before operation")
        
        if equipment_type == "crane":
            recommendations.append("Establish ground guides and maintain clear communication")
            recommendations.append("Monitor wind conditions - suspend operations if winds exceed 20 mph")
        
        if operation_info["duration_hours"] > 8:
            recommendations.append("Plan operator rotation to prevent fatigue")
            recommendations.append("Schedule equipment breaks every 2 hours for inspection")
        ...
        return json.dumps({
            ...
            "operational_readiness": {
                ...
                "blocking_issues": [
                    alert["message"] for alert in safety_alerts 
...
def debris_removal_coordinator(
    area_id: str = "AREA-A",
    removal_priority: Literal["immediate", "planned", "cleanup", "hazardous"] = "immediate",
    debris_type: Literal["structural", "mixed", "hazardous", "organic", "metal"] = "mixed",
    volume_estimate_cubic_yards: Optional[float] = None,
    ...
    try:
        ...
        debris_specifications = {
            ...
            "mixed": {
                ...
                "safety_concerns": ["unknown_materials", "sharp_objects", "contamination"],
            ...
            "hazardous": {
                ...
                "safety_concerns": ["chemical_exposure", "contamination", "special_handling"],
                "equipment_needed": ["specialized_containers", "hazmat_suits", "monitoring_equipment"],
            ...
            "metal": {
                ...
                "safety_concerns": ["sharp_edges", "heavy_pieces", "structural_failure"],
        ...
        if volume_estimate_cubic_yards:
            estimated_time_hours = (volume_estimate_cubic_yards / debris_spec["processing_rate_cy_hour"]) * priority_spec["resources_multiplier"]
            trucks_needed = max(2, int(volume_estimate_cubic_yards / 20))  # 20 cy per truck load
        else:
        ...
        removal_data = {
            "area_identification": {
                ...
                "disposal_site_status": "identified" if disposal_site_identified else "pending"
            },
            ...
            "safety_protocols": {
                ...
                "monitoring_requirements": determine_monitoring_requirements(debris_type),
                "emergency_procedures": generate_debris_emergency_procedures(debris_type)
            },
        ...
        operation_phases = [
            {
                ...
                "duration_hours": 2,
                "activities": ["Access route clearing", "Equipment positioning", "Safety setup"],
            ...
            },
            {
                ...
                "activities": ["Debris removal", "Sorting operations", "Loading operations"],
            ...
            },
            {
                ...
                "activities": ["Area sweeping", "Final inspection", "Equipment cleanup"],
            ...
            },
            {
                "phase": "Documentation",
                "duration_hours": 1,
                "activities": ["Final documentation", "Environmental compliance", "Handoff procedures"],
        ...
        if not access_route_clear:
            blocking_issues.append("Access routes must be cleared before debris removal can begin")
        ...
        if debris_type == "hazardous" and "hazmat_specialists" not in base_personnel:
            blocking_issues.append("Hazmat specialists required for hazardous debris removal")
        
        recommendations = []
        
        if removal_priority == "immediate":
            recommendations.append("Deploy maximum available resources for immediate debris removal")
            recommendations.append("Establish 24-hour operations if necessary")
        
        if debris_type == "hazardous":
            recommendations.append("Coordinate with Environmental Specialist for hazmat protocols")
            recommendations.append("Implement continuous air quality monitoring")
        
        if not access_route_clear:
            recommendations.append("Priority: Clear access routes for equipment movement")
        
        if volume_estimate_cubic_yards > 500:
            recommendations.append("Consider establishing temporary debris staging area")
            recommendations.append("Coordinate with multiple disposal sites to manage volume")
...
def determine_operation_complexity(operation_type: str, personnel: int, victim_id: Optional[str]) -> str:
...
def assess_operation_safety_level(operation_type: str, location: str) -> str:
    ...
    if "floor" in location.lower() and any(num in location for num in ["2", "3", "4", "5"]):
...
def generate_extraction_safety_considerations(entrapment_type: str, victim_condition: str, access_difficulty: str) -> List[str]:
...
def generate_stabilization_timeline(estimated_time: float, materials: List[str], personnel: int) -> List[Dict[str, Any]]:
    ...
    phases = [
        ...
        },
        {
            ...
            "duration_hours": 0.5,
            "personnel": personnel,
            "activities": ["Position equipment", "Establish safety perimeter", "Initial assessment"]
        },
        {
            ...
            "personnel": personnel,
            "activities": ["Install stabilization", "Monitor progress", "Quality checks"]
        },
        {
...
def determine_adequate_stabilization_method(load_requirements: float, methods: Dict[str, Dict]) -> str:
...
def generate_heavy_equipment_safety_protocols(equipment_type: str, operation_mode: str, safety_radius: int) -> Dict[str, Any]:
...
def determine_ppe_requirements(debris_type: str) -> List[str]:
    ...
    base_ppe = ["Hard hat", "Safety glasses", "Steel-toed boots", "High-visibility vest"]
    
    type_specific = {
        "hazardous": ["Chemical-resistant suit", "Self-contained breathing apparatus", "Chemical-resistant gloves"],
...
def determine_monitoring_requirements(debris_type: str) -> List[str]:
    ...
    base_monitoring = ["Personnel accountability", "Equipment status", "Weather conditions"]
    
    type_specific = {
        "hazardous": ["Air quality monitoring", "Chemical exposure monitoring", "Contamination levels"],
...
def generate_quality_control_points(debris_type: str) -> List[str]:
    ...
    type_specific = {
        "hazardous": ["Hazmat classification verification", "Chain of custody documentation"],
