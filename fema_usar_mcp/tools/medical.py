...
def patient_care_tracker(
    ...
    triage_priority: Literal["immediate", "delayed", "minor", "deceased", "expectant"] = "delayed",
    ...
    try:
        ...
        care_level = determine_care_level(triage_priority, treatments_given, medications_administered)
        ...
        time_to_treatment = calculate_time_to_treatment(estimated_discovery_time, current_time)
        ...
        medical_assessment = {
            ...
            "secondary_survey": {
                ...
                "mechanism_of_injury": determine_mechanism_of_injury(location_found, chief_complaint),
                "pain_scale": determine_pain_level(chief_complaint, triage_priority),
                "additional_findings": generate_additional_findings(treatments_given, medications_administered)
            }
        }
        ...
        ics_213_data = {
            ...
            "medical_information": {
                ...
                "medications_given": format_medications_for_ics(medications_administered),
                "patient_condition": determine_patient_condition(triage_priority, vital_signs)
            },
            
            "transport_information": {
                "transport_priority": triage_priority,
                "destination": transport_destination,
                "transport_mode": determine_transport_mode(triage_priority, location_found),
                "eta_to_hospital": calculate_transport_eta(transport_destination, triage_priority)
            },
        ...
        supply_usage = calculate_medical_supply_usage(treatments_given, medications_administered)
        ...
        patient_tracking = {
            ...
            "medical_status": {
                "current_condition": determine_patient_condition(triage_priority, vital_signs),
                "stability": assess_patient_stability(vital_signs, triage_priority),
                "transport_ready": assess_transport_readiness(treatments_given, triage_priority),
                "estimated_transport_time": calculate_transport_eta(transport_destination, triage_priority)
            },
            ...
            "coordination_requirements": {
                "specialist_consultation": triage_priority in ["immediate", "expectant"],
                "advanced_life_support": care_level in ["als", "critical"],
                "helicopter_transport": triage_priority == "immediate" and "remote" in location_found.lower(),
        ...
        if not vital_signs:
            medical_recommendations.append("Obtain full set of vital signs if patient condition permits")
        
        if len(medications_administered) == 0 and triage_priority in ["immediate", "delayed"]:
            medical_recommendations.append("Consider pain management if not contraindicated")
        
        # Supply and resource recommendations
        if supply_usage.get("critical_supplies_used", 0) > 5:
            medical_recommendations.append("Restock critical medical supplies after patient transport")
        
        medical_recommendations.extend([
            f"Complete ICS-213 form and submit to Medical Unit Leader",
            f"Coordinate patient handoff with transport team",
        ...
        return json.dumps({
            ...
            "critical_actions": generate_critical_actions(triage_priority, care_level, treatments_given),
            "next_steps": [
...
def medical_supply_inventory(
    inventory_action: Literal["check_levels", "consumption_update", "restock_request", "audit"] = "check_levels",
    supply_category: Literal["medications", "equipment", "consumables", "controlled_substances", "all"] = "all",
    ...
    try:
        ...
        medical_supplies = {
            "medications": {
                "pain_management": {
                    "morphine_10mg": {"stock": 45, "max_capacity": 50, "critical": True, "controlled": True},
                    "ibuprofen_800mg": {"stock": 180, "max_capacity": 200, "critical": False, "controlled": False},
                    "acetaminophen_1g": {"stock": 220, "max_capacity": 250, "critical": False, "controlled": False},
                    "fentanyl_patches": {"stock": 8, "max_capacity": 20, "critical": True, "controlled": True}
                },
                "cardiac": {
                    "epinephrine_1mg": {"stock": 12, "max_capacity": 15, "critical": True, "controlled": False},
                    "atropine_1mg": {"stock": 8, "max_capacity": 10, "critical": True, "controlled": False},
                    "aspirin_325mg": {"stock": 95, "max_capacity": 100, "critical": False, "controlled": False},
                    "nitroglycerin_spray": {"stock": 6, "max_capacity": 8, "critical": True, "controlled": False}
                },
                "respiratory": {
                    "albuterol_inhaler": {"stock": 14, "max_capacity": 15, "critical": True, "controlled": False},
                    "benadryl_50mg": {"stock": 48, "max_capacity": 50, "critical": False, "controlled": False},
                    "dexamethasone_4mg": {"stock": 22, "max_capacity": 25, "critical": True, "controlled": False}
                }
            },
            
            "equipment": {
                "monitoring": {
                    "pulse_oximeters": {"stock": 6, "max_capacity": 8, "critical": True, "controlled": False},
                    "bp_cuffs": {"stock": 8, "max_capacity": 10, "critical": True, "controlled": False},
                    "thermometers": {"stock": 12, "max_capacity": 15, "critical": False, "controlled": False},
                    "glucose_meters": {"stock": 4, "max_capacity": 5, "critical": True, "controlled": False}
                },
                "airway": {
                    "bag_valve_masks": {"stock": 8, "max_capacity": 10, "critical": True, "controlled": False},
                    "oral_airways": {"stock": 45, "max_capacity": 50, "critical": True, "controlled": False},
                    "nasal_airways": {"stock": 38, "max_capacity": 40, "critical": True, "controlled": False},
                    "oxygen_tanks": {"stock": 6, "max_capacity": 8, "critical": True, "controlled": False}
                },
                "trauma": {
                    "spine_boards": {"stock": 4, "max_capacity": 6, "critical": True, "controlled": False},
                    "cervical_collars": {"stock": 18, "max_capacity": 20, "critical": True, "controlled": False},
                    "splints": {"stock": 28, "max_capacity": 30, "critical": True, "controlled": False},
                    "stretchers": {"stock": 8, "max_capacity": 10, "critical": True, "controlled": False}
                }
            },
            
            "consumables": {
                "wound_care": {
                    "gauze_4x4": {"stock": 180, "max_capacity": 200, "critical": False, "controlled": False},
                    "trauma_dressings": {"stock": 45, "max_capacity": 50, "critical": True, "controlled": False},
                    "elastic_bandages": {"stock": 38, "max_capacity": 40, "critical": False, "controlled": False},
                    "medical_tape": {"stock": 25, "max_capacity": 30, "critical": False, "controlled": False}
                },
                "iv_supplies": {
                    "iv_catheters_18g": {"stock": 48, "max_capacity": 50, "critical": True, "controlled": False},
                    "iv_bags_ns": {"stock": 35, "max_capacity": 40, "critical": True, "controlled": False},
                    "iv_tubing": {"stock": 42, "max_capacity": 45, "critical": True, "controlled": False},
                    "syringes_10ml": {"stock": 85, "max_capacity": 100, "critical": False, "controlled": False}
                }
            }
        }
        
        # Calculate current inventory status
        inventory_status = analyze_inventory_status(medical_supplies, supply_category, low_stock_threshold)
        ...
        inventory_data = {
            ...
            "supply_categories": filter_inventory_by_category(medical_supplies, supply_category),
            ...
            "alerts_and_warnings": generate_inventory_alerts(inventory_status, usage_data),
            
            "restock_recommendations": generate_restock_recommendations(inventory_status, usage_data),
            
            "controlled_substance_tracking": track_controlled_substances(medical_supplies, usage_data) if supply_category in ["medications", "controlled_substances", "all"] else None
        }
        ...
        if inventory_action == "consumption_update":
            inventory_data["consumption_update"] = {
                ...
                "remaining_stock": calculate_remaining_stock(medical_supplies, usage_data),
                "critical_consumption": identify_critical_consumption(usage_data),
                "replacement_needed": identify_replacement_needs(inventory_status, usage_data)
            }
        
        elif inventory_action == "restock_request":
            inventory_data["restock_request"] = {
                "priority_items": inventory_status["low_stock_items"] + inventory_status["out_of_stock_items"],
                "order_quantities": calculate_reorder_quantities(inventory_status, usage_data),
            ...
            inventory_data["audit_results"] = {
                ...
                "security_compliance": assess_controlled_substance_security(medical_supplies)
            }
        ...
        if inventory_status["out_of_stock_items"]:
            recommendations.append(f"URGENT: {len(inventory_status['out_of_stock_items'])} critical items out of stock")
            recommendations.append("Initiate emergency resupply procedures immediately")
        
        if inventory_status["low_stock_items"]:
            recommendations.append(f"Restock {len(inventory_status['low_stock_items'])} items below threshold")
        
        if usage_data["high_usage_items"]:
            recommendations.append("Monitor high-usage items for potential early depletion")
        
        if supply_category in ["controlled_substances", "medications", "all"]:
            recommendations.append("Verify controlled substance security and documentation")
        ...
        return json.dumps({
            ...
            "immediate_actions": [
                action for action in [
                    "Address out-of-stock critical items" if inventory_status["out_of_stock_items"] else None,
                    "Process restock requests" if inventory_status["low_stock_items"] else None,
                    "Verify controlled substance counts" if supply_category in ["controlled_substances", "medications", "all"] else None,
                    "Update consumption records" if inventory_action == "consumption_update" else None
                ] if action
            ]
        }, indent=2)
...
def triage_coordinator(
    ...
    operation_mode: Literal["initial_triage", "ongoing_operations", "mass_casualty", "demobilization"] = "ongoing_operations",
    patient_flow_rate: int = 3,
    triage_personnel_available: int = 4,
    transport_availability: Literal["immediate", "limited", "delayed", "unavailable"] = "immediate",
    receiving_hospital_capacity: Literal["open", "limited", "closed", "divert"] = "open"
) -> str:
    ...
    try:
        ...
        triage_capacity = calculate_triage_capacity(triage_personnel_available, operation_mode)
        
        # Assess resource requirements
        resource_requirements = assess_triage_resource_needs(current_census, triage_capacity, transport_availability)
        ...
        patient_flow = {
            ...
            "flow_rate": {
                ...
                "capacity_utilization": (patient_flow_rate / triage_capacity["max_hourly_capacity"]) * 100,
                "bottlenecks": identify_triage_bottlenecks(current_census, transport_availability, receiving_hospital_capacity)
            },
            "transport_coordination": {
                ...
                "estimated_clear_time": calculate_triage_clear_time(current_census, transport_availability)
            }
        }
        ...
        triage_data = {
            ...
            "patient_management": {
                ...
                "priority_queue": generate_priority_queue(current_census, triage_categories)
            },
            ...
            "hospital_coordination": {
                ...
                "bed_availability": assess_hospital_bed_availability(receiving_hospital_capacity),
                "specialist_availability": assess_specialist_availability(current_census),
                "diversion_protocols": generate_diversion_protocols(receiving_hospital_capacity, current_census)
            },
            
            "quality_metrics": {
                "triage_accuracy": 95.0,  # Percentage
                "average_triage_time": calculate_average_triage_time(operation_mode, triage_personnel_available),
        ...
        if immediate_patients > 0:
            critical_actions.append(f"IMMEDIATE TRANSPORT NEEDED: {immediate_patients} critical patients")
        ...
        if transport_availability == "unavailable":
            critical_actions.append("NO TRANSPORT AVAILABLE: Establish on-site treatment capability")
        ...
        return json.dumps({
            ...
            "data": triage_data,
            "recommendations": recommendations,
            "critical_actions": critical_actions or ["Continue current triage operations"],
            "coordination_requirements": {
                ...
                "logistics_support": patient_flow["flow_rate"]["capacity_utilization"] > 80
            }
        }, indent=2)
...
def health_surveillance(
    monitoring_scope: Literal["task_force_personnel", "incident_responders", "affected_population", "environmental"] = "task_force_personnel",
    surveillance_type: Literal["routine", "outbreak_investigation", "environmental_exposure", "occupational_health"] = "routine",
    ...
    try:
        ...
        health_parameters = {
            "vital_signs": ["temperature", "blood_pressure", "pulse", "respiratory_rate", "oxygen_saturation"],
            "environmental_factors": ["air_quality", "temperature", "humidity", "noise_level", "radiation", "chemical_exposure"],
            "occupational_hazards": ["heat_stress", "dehydration", "fatigue", "injury_risk", "respiratory_exposure"],
            "infectious_disease": ["respiratory_symptoms", "gastrointestinal_symptoms", "fever", "rash", "exposure_history"]
        }
        
        # Assess current health status
        current_health_status = assess_personnel_health_status(monitoring_scope, personnel_health_status)
        
        # Environmental health assessment
        environmental_assessment = assess_environmental_health_risks(environmental_hazards, surveillance_time)
        
        # Generate health alerts
        health_alerts = generate_health_alerts(current_health_status, environmental_assessment, alert_threshold)
        
        # Calculate health metrics
        health_metrics = calculate_health_surveillance_metrics(current_health_status, environmental_assessment)
        ...
        surveillance_data = {
            ...
            "surveillance_activities": {
                ...
                "exposure_assessments": environmental_assessment["assessments_completed"]
            },
        ...
        if surveillance_type == "outbreak_investigation":
            surveillance_data["outbreak_investigation"] = {
                "suspected_cases": identify_suspected_outbreak_cases(current_health_status),
                "contact_tracing": perform_contact_tracing(current_health_status),
                "isolation_recommendations": generate_isolation_recommendations(current_health_status),
            ...
            surveillance_data["exposure_assessment"] = {
                ...
                "health_effects_monitoring": monitor_exposure_health_effects(current_health_status),
                "mitigation_effectiveness": assess_mitigation_effectiveness(environmental_assessment)
            }
        
        elif surveillance_type == "occupational_health":
            surveillance_data["occupational_health"] = {
                "work_related_injuries": track_work_related_injuries(current_health_status),
                "ergonomic_assessments": perform_ergonomic_assessments(),
                "ppe_compliance": assess_ppe_compliance(current_health_status),
                "workplace_hazard_controls": evaluate_hazard_controls(environmental_assessment)
            }
        ...
        if health_alerts:
            recommendations.append(f"HEALTH ALERT: {len(health_alerts)} active health alerts require attention")
        ...
        return json.dumps({
            ...
            "recommendations": recommendations,
            "immediate_actions": [
                action["recommended_action"] for action in health_alerts 
            ...
            "reporting_schedule": {
                ...
                "report_recipients": determine_health_report_recipients(surveillance_type),
...
def evacuation_coordinator(
    evacuation_type: Literal["medical", "casualty", "personnel", "mass_casualty"] = "medical",
    transport_mode: Literal["ground_ambulance", "helicopter", "fixed_wing", "multiple_modes"] = "ground_ambulance",
    destination_type: Literal["local_hospital", "trauma_center", "specialty_facility", "military_hospital"] = "local_hospital",
    patient_acuity: Literal["critical", "urgent", "stable", "mixed"] = "urgent",
    weather_conditions: Literal["clear", "marginal", "poor", "severe"] = "clear",
    transport_distance_miles: Optional[float] = None,
    ...
    try:
        ...
        transport_capabilities = {
            "ground_ambulance": {
                ...
                "medical_equipment": ["basic_als", "monitoring", "oxygen", "medications"],
            ...
            "helicopter": {
                ...
                "medical_equipment": ["advanced_als", "ventilator", "blood_products", "surgical_capability"],
            ...
            "fixed_wing": {
                ...
                "medical_equipment": ["icu_level", "ventilator", "blood_products", "surgical_suite"],
        ...
        transport_feasibility = assess_transport_feasibility(
            transport_mode, weather_conditions, transport_distance_miles, transport_capabilities
        )
        
        # Calculate transport timeline
        transport_timeline = calculate_transport_timeline(
            transport_mode, transport_distance_miles, weather_conditions, transport_capabilities
        )
        
        # Determine destination facility capabilities
        destination_capabilities = assess_destination_capabilities(destination_type, patient_acuity)
        
        # Generate patient preparation requirements
        patient_preparation = generate_patient_preparation_requirements(
            patient_acuity, transport_mode, transport_distance_miles, special_requirements
        )
        ...
        evacuation_data = {
            ...
            "transport_plan": {
                ...
                "backup_transport": determine_backup_transport(transport_mode, weather_conditions),
                "transport_feasibility": transport_feasibility,
                "timeline": transport_timeline,
                "route_planning": generate_route_planning(transport_mode, transport_distance_miles, weather_conditions)
            },
            
            "destination_coordination": {
                "primary_destination": destination_type,
                "destination_capabilities": destination_capabilities,
                "bed_availability": assess_destination_bed_availability(destination_type),
                "specialist_availability": assess_destination_specialists(destination_type, patient_acuity),
                "arrival_notification": generate_arrival_notification_requirements(destination_type, patient_acuity)
            },
            ...
            "resource_coordination": {
                ...
                "estimated_cost": calculate_evacuation_cost(transport_mode, transport_distance_miles, resource_requirements)
            },
            
            "communication_plan": {
                "pre_transport": generate_pre_transport_communications(destination_type, patient_acuity),
                "during_transport": generate_transport_communications(transport_mode, transport_distance_miles),
                "arrival_coordination": generate_arrival_communications(destination_type),
                "emergency_communications": generate_emergency_communication_protocols(transport_mode)
            }
        }
        ...
        if evacuation_type == "mass_casualty":
            evacuation_data["mass_casualty_coordination"] = {
                "patient_distribution": plan_patient_distribution(transport_capabilities, destination_capabilities),
                "transport_sequencing": sequence_mass_casualty_transports(patient_acuity),
                "resource_multiplication": calculate_mass_casualty_resources(resource_requirements),
        ...
        if weather_conditions in ["marginal", "poor", "severe"]:
            evacuation_data["weather_considerations"] = {
                "transport_limitations": assess_weather_transport_limitations(transport_mode, weather_conditions),
                "safety_protocols": generate_weather_safety_protocols(weather_conditions),
                "backup_planning": generate_weather_backup_plans(transport_mode, weather_conditions),
                "delay_probabilities": calculate_weather_delay_probabilities(weather_conditions)
            }
        ...
        return json.dumps({
            ...
            "go_no_go_decision": {
                ...
                "decision_factors": transport_feasibility.get("decision_factors", []),
                "approval_authority": determine_evacuation_approval_authority(evacuation_type, patient_acuity)
            }
        }, indent=2)
...
def determine_care_level(triage_priority: str, treatments: List[str], medications: List[Dict[str, Any]]) -> str:
    """Determine care level based on triage and interventions."""
    if triage_priority == "immediate" or any("advanced" in str(med) for med in medications):
        return "critical"
...
def calculate_time_to_treatment(discovery_time: datetime, treatment_time: datetime) -> int:
...
def assess_breathing_status(vital_signs: Dict[str, Any], treatments: List[str]) -> str:
    ...
    if vital_signs.get("respiratory_rate", 16) < 10 or vital_signs.get("respiratory_rate", 16) > 24:
        return "abnormal"
...
def assess_circulation_status(vital_signs: Dict[str, Any], treatments: List[str]) -> str:
    """Assess circulation status."""
    if vital_signs.get("heart_rate", 80) > 100 or vital_signs.get("blood_pressure_systolic", 120) < 90:
        return "compromised"
...
def generate_additional_findings(treatments: List[str], medications: List[Dict[str, Any]]) -> List[str]:
...
def format_vital_signs_for_ics(vital_signs: Dict[str, Any]) -> str:
    ...
    if vital_signs.get("blood_pressure_systolic"):
        formatted.append(f"BP: {vital_signs['blood_pressure_systolic']}/{vital_signs.get('blood_pressure_diastolic', 80)}")
...
def format_treatments_for_ics(treatments: List[str]) -> str:
    ...
    return "; ".join(treatments[:5]) + (f" and {len(treatments)-5} more" if len(treatments) > 5 else "")
...
def calculate_medical_supply_usage(treatments: List[str], medications: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def generate_critical_actions(triage: str, care_level: str, treatments: List[str]) -> List[str]:
...
def analyze_inventory_status(supplies: Dict, category: str, threshold: float) -> Dict[str, Any]:
...
def generate_triage_census(mode: str, flow_rate: int) -> Dict[str, int]:
    ...
    if mode == "mass_casualty":
        return {"immediate": 8, "delayed": 15, "minor": 25, "expectant": 3, "deceased": 2}
    else:
