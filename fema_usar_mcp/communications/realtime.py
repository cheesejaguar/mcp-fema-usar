...
class MessageBroker:
    ...
    def add_connection(self, connection: WebSocketConnection):
        ...
        logger.info(f"Added WebSocket connection: {connection.id} (user: {connection.user_id})")
    ...
    def remove_connection(self, connection_id: str):
        ...
        if connection_id in self.connections:
            ...
            logger.info(f"Removed WebSocket connection: {connection_id} (user: {connection.user_id})")
    ...
    async def publish_message(self, message: Message):
        ...
        if self.redis_client and message.requires_ack:
            ...
            await self.redis_client.expire(f"messages:{message.channel}", 3600)  # 1 hour
    ...
    async def handle_websocket_message(self, connection: WebSocketConnection, data: str):
    ...
    def _can_subscribe(self, connection: WebSocketConnection, channel: str) -> bool:
        ...
        if channel == Channel.COMMAND and connection.user_role not in ["task_force_leader", "operations_chief"]:
        ...
        while self.running:
            try:
                ...
                for connection_id in stale_connections:
                    if connection_id in self.connections:
                        await self.connections[connection_id].close(1001, "Connection timeout")
...
# Factory function
def create_communication_system(redis_url: Optional[str] = None) -> MessageBroker:
