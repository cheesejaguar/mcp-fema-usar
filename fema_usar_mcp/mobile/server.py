...
from ..core import USARTaskForceConfig, OperationalStatus
from ..monitoring.metrics import USARMetrics, MonitoringManager
...
class MobileConfig:
    ...
    def __init__(self):
        ...
        self.bandwidth_limited = os.getenv("BANDWIDTH_LIMITED", "true").lower() == "true"
        self.battery_optimization = os.getenv("BATTERY_OPTIMIZATION", "true").lower() == "true"
...
class MobileServer:
    ...
    def __init__(self):
        ...
        self.sync_manager: Optional[MobileSyncManager] = None
        self.gps_manager: Optional[GPSManager] = None
        self.battery_manager: Optional[BatteryManager] = None
        self.offline_cache: Optional[OfflineCache] = None
    ...
    def _setup_routes(self):
        ...
        async def get_network_quality():
            ...
            return {"quality": quality, "timestamp": datetime.now(timezone.utc).isoformat()}
        ...
        while True:
            try:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (
                    battery_level, gps_lat, gps_lon, self.network_quality,
                    "active" if self.sync_manager else "inactive", cache_size_mb
                ))
        ...
        while True:
            try:
                ...
                if self.sync_manager:
                    ...
                    if hq_reachable:
                        self.last_hq_contact = datetime.now(timezone.utc)
