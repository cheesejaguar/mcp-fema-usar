...
class NIMSICSPosition(BaseModel):
    ...
    reports_to: str | None = Field(None, description="Supervisor position ID")
    assigned_person: str | None = Field(None, description="Assigned person name")
    qualifications: list[str] = Field(
        default_factory=list, description="Required qualifications"
    )
    responsibilities: list[str] = Field(
        default_factory=list, description="Position responsibilities"
    )
...
class NIMSOperationalPeriod(BaseModel):
    ...
    strategies: list[str] = Field(
        default_factory=list, description="Strategies employed"
    )
    ...
    safety_concerns: list[str] = Field(
        default_factory=list, description="Safety concerns"
    )
...
class NIMSICTClient:
    ...
    async def get_incident_organization(
        self, incident_id: str
    ) -> list[NIMSICSPosition]:
        ...
        try:
            ...
            positions = [
                NIMSICSPosition(**pos) for pos in org_data.get("positions", [])
            ]

            logger.info(
                f"Retrieved {len(positions)} ICS positions for incident {incident_id}"
            )
            return positions
    ...
    async def update_position_assignment(
        ...
        try:
            ...
            logger.info(
                f"Updated ICS position {position_id} assignment to {assigned_person}"
            )
            return True
    ...
    async def submit_resource_order(
        ...
        try:
            ...
            response = await self.session.post(
                f"/incidents/{incident_id}/resource-orders", json=order_data
            )
            response.raise_for_status()
    ...
    async def create_operational_period(
        ...
        try:
            ...
            response = await self.session.post(
                f"/incidents/{incident_id}/operational-periods", json=period_data
            )
            response.raise_for_status()
            ...
            logger.info(
                f"Created NIMS operational period: {operational_period.period_id}"
            )
    ...
    async def submit_situation_report(
        self, incident_id: str, report_type: str, report_data: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            response = await self.session.post(
                f"/incidents/{incident_id}/reports", json=report_payload
            )
            response.raise_for_status()
...
class NIMSICTIntegrationManager:
    ...
    def __init__(self, credentials: NIMSICTCredentials):
        ...
        """
        self.credentials = credentials
        self._client: NIMSICTClient | None = None
    ...
    async def sync_incident_organization(
        self, incident_id: str, task_force_personnel: list[dict[str, Any]]
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Synchronized {len(task_force_personnel)} personnel with NIMS ICS"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to sync incident organization with NIMS ICT: {str(e)}"
            )
            return False

    def _map_to_ics_position(
        self, person: dict[str, Any], ics_positions: list[NIMSICSPosition]
    ) -> NIMSICSPosition | None:
        ...
        for ics_pos in ics_positions:
            if (
                ics_pos.position_title.lower().replace(" ", "_") == mapped_title
                and ics_pos.status == "vacant"
            ):
                return ics_pos
