...
class IRISResource(BaseModel):
    ...
    capabilities: list[str] = Field(
        default_factory=list, description="Resource capabilities"
    )
    availability_date: datetime | None = Field(
        None, description="Available deployment date"
    )
...
class IRISDeploymentOrder(BaseModel):
    ...
    resources_requested: list[dict[str, Any]] = Field(
    ...
    special_instructions: str | None = Field(
        None, description="Special deployment instructions"
    )
...
class FEMAIRISClient:
    ...
    async def _ensure_authenticated(self):
        ...
        if datetime.now(UTC).timestamp() + 300 > self._token_expires:
            await self._authenticate()

    async def get_available_resources(
        self,
        resource_type: IRISResourceType | None = None,
        ...
        if location_radius_miles and center_lat and center_lon:
            params.update(
                {"radius": location_radius_miles, "lat": center_lat, "lon": center_lon}
            )
        if capabilities:
        ...
        try:
            ...
            resources = [
                IRISResource(**resource)
                for resource in resources_data.get("resources", [])
            ]

            logger.info(
                f"Retrieved {len(resources)} available resources from FEMA IRIS"
            )
            return resources
    ...
    async def request_resource_deployment(
        ...
        try:
            ...
            logger.info(
                f"Created FEMA IRIS deployment order: {deployment_order.order_id}"
            )
    ...
    async def update_resource_status(
        ...
        try:
            response = await self.session.put(
                f"/resources/{resource_id}/status", json=update_data
            )
            response.raise_for_status()

            logger.info(
                f"Updated FEMA IRIS resource {resource_id} status to {status.value}"
            )
            return True
    ...
    async def submit_situation_report(
        self, incident_id: str, report_data: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Submitted situation report to FEMA IRIS for incident {incident_id}"
            )
            return True
...
class IRISIntegrationManager:
    ...
    def __init__(self, credentials: IRISCredentials):
        ...
        """
        self.credentials = credentials
        self._client: FEMAIRISClient | None = None
    ...
    async def sync_task_force_resources(
        self, task_force_id: str, resources: list[dict[str, Any]]
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Synchronized {len(resources)} resources for {task_force_id} with FEMA IRIS"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to sync task force resources with FEMA IRIS: {str(e)}"
            )
            return False

    def _convert_to_iris_resource(
        self, task_force_id: str, resource: dict[str, Any]
    ) -> dict[str, Any]:
...
def create_iris_integration(
    api_key: str, secret_key: str, base_url: str = "https://iris.fema.gov/api/v2"
