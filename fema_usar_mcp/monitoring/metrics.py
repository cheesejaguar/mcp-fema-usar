...
def create_system_alert_rules(alert_manager: AlertManager):
    ...
    def high_cpu_usage(metrics: Dict[str, Any]) -> Optional[Alert]:
    ...
    def high_memory_usage(metrics: Dict[str, Any]) -> Optional[Alert]:
    ...
    def disk_space_low(metrics: Dict[str, Any]) -> Optional[Alert]:
        ...
        for mount_point, usage_percent in disk_usage.items():
            if usage_percent > 95:
                return Alert(
                    ...
                    source="system",
                    timestamp=datetime.now(timezone.utc),
                    labels={"component": "system", "metric": "disk", "mount_point": mount_point}
                )
            elif usage_percent > 85:
                return Alert(
...
def create_usar_alert_rules(alert_manager: AlertManager):
    ...
    def deployment_health_check(metrics: Dict[str, Any]) -> Optional[Alert]:
    ...
    def safety_incident_spike(metrics: Dict[str, Any]) -> Optional[Alert]:
    ...
    def equipment_failure_rate(metrics: Dict[str, Any]) -> Optional[Alert]:
